...
> **状态**：**Phase 1 + Phase 2 路径 A + Phase 4 诊断页均已实施并发布 v1.3.4**（真机验证路线见 3.1：OPPO 官方云真机待实名审核）
> **基础**：v1.3.3（versionCode 12，`minSdk 24` / `targetSdk 36`）→ **v1.3.4（versionCode 13，`targetSdk 35`）**
...
- [x] **内置「问题诊断」页（已随 v1.3.4 发布）** —— 不再需要单独出诊断包：
  - 入口：**我的 → 关于 → 问题诊断（触摸自检）**（隐藏路由 `/diagnostics`，不占底部导航）
  - 内容：触摸测试区（tap 计数 + 落点坐标，判断触摸能不能到达 JS）、**JS 心跳**（数字不涨 = JS 线程卡死）、
    设备（厂商/品牌/机型/Android 版本/uiMode）、显示与安全区（window/screen/scale/fontScale/insets）、
    运行时（首个触摸时刻、边缘横滑开关、令牌读取是否超时、待同步条数）
  - 结论：直接把三类成因判开 —— ① 触摸到不了 JS（ROM 层拦截）② 触摸到了但被覆盖层吃掉（UI 层问题）
    ③ JS 线程卡死；并给出一键「复制 / 分享诊断信息」（`Share.share`，零新依赖）
  - 纯逻辑在 `src/lib/diagnostics.ts`（`judgeTouch` / `formatDiagnostics`，零 RN import，10 条单测）
- [ ] **客户端日志上报**（按需）：`POST /api/internal/client-log` 收启动/首个触摸/异常堆栈 —— 本轮未做（诊断页已能覆盖判断，避免新增表与端点）
- [x] **给内测用户的一页「ColorOS 小贴士」**：见下方附录 A（可直接复制发给内测用户）

---

# 附录 A · 内测复测指引（可直接发给内测用户）

> **【苦旅 v1.3.4 复测请求 · 大约 2 分钟】**
>
> 之前你反馈「装好后能打开，但屏幕上点哪儿都没反应」，这次专门做了适配修复，麻烦按下面三步复测一下：
>
> **1. 更新到 v1.3.4**
> - 打开 App →「我的」→「检查更新」→ 按提示下载安装；或直接用手机浏览器打开 `https://learn.yuanabd.cn/download.html` 下载
> - 装完请确认「我的 → 关于」显示 **v1.3.4**；若设置里显示的还是旧版本，说明装的还是旧包，麻烦重装一次
>
> **2. 依次试这 5 件事，告诉我哪一步不行**
> ① 底部五个 Tab（今日 / 学习 / 职业 / 健康 / 我的）能不能切换
> ② 首页和「学习」页能不能上下滚动
> ③ 随便点开一个卡片 / 弹层，能不能正常打开和关闭
> ④ 「我的」→ 检查更新，按钮有没有反应
> ⑤ 用「我的 → 关于 → 问题诊断」里的触摸测试区，点几下数字会不会涨
>
> **3. 如果还是点不动，请把这份信息发我（最关键的一步）**
> - 打开「我的」→ 关于 → **问题诊断（触摸自检）**
> - 在那个大框里点几下 → 点「复制 / 分享诊断信息」→ 发给我
> - 同时告诉我你的**手机型号**和 **ColorOS 版本**（设置 → 关于手机），以及是否有装**悬浮球 / 录屏 / 无障碍类** App
>
> **顺手可以检查的 ColorOS 设置（只影响你这台手机，不装也能用）**
> - 设置 → 应用管理 → 苦旅 → **关闭「应用兼容模式 / 全屏显示」相关的兼容开关**（如果有）
> - 设置 → 电池 → 应用耗电管理 → 苦旅 → 允许后台运行、关闭「应用速冻」
> - 不要把 App 放在**小窗 / 分屏**里打开（先全屏试）
> - 若有「应用分身 / 儿童空间 / 隐私空间」，先在本体空间中试
