...
- **改动**：
  1. **运动图标**：羽毛球/排球/乒乓球/跑步/跳绳/拳击/瑜伽等细粒度图标修复；动效简化为放大回弹（跑步/跳绳等通用放大缩小）。
  2. **底部 Dock**：选中态由灰色阴影改为方形切换；收窄 dock 宽度（不与手机两侧紧贴）；学习页面 dock 图标畸形修复（半边消失问题）；dock 改为不透明背景。
  3. **学习阶段卡片**：点击阶段后其他卡片内容不消失；当前选中卡片加流光特效（方框周围突出）；更多阶段半屏抽屉支持长按拖拽调整顺序 + 底部横线跟随手指轨迹。
  4. **登录卡片**：重设计登录卡片组件；退出按钮不再溢出屏幕；整体优化美观度。
  5. **首页 UI**：卡片内白条修复；增加呼吸光晕特效突出焦点；符合油画风格主题。
  6. **打包发布**：app.json 版本 1.1.3→1.1.4，versionCode 5→6；APK 构建并上传至服务器；重新生成下载页 QR 码。
- **涉及文件**：apps/mobile/app.json；docs/改动记录与任务看板.md；docs/Learn-Workbench-2.0-改动记录.md。
- **验证**：APK 构建成功（63MB），签名 MD5 3057105285981cc18597a95c1370c147 与备案一致；APK 上传至 /data/learn-workbench/releases/；QR 码重新生成并部署。
- **影响**：版本 1.1.4（versionCode 6），新 APK 可通过 https://learn.yuanabd.cn/download.html 扫码下载。
