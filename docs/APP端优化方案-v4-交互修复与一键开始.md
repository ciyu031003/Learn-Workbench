...
> | P2 | 一键开始大按钮 + Bing 默认壁纸 + keyboard-controller 键盘适配 + 禁明文 | ✅ | `57eaff3` |
> | P3 | 训练记录重做（动作可删/弹层重做/选择面板）+ 健身房动作字典（迁移 046 + /api/exercises） | ✅ | `2a9358c` |
> | P4 | 饮食 日/周/月 + 月历 + 「吃一点」UI（含进阶套件） | ⏳ | — |
>
> P1 验证：`pnpm -F mobile test` 22 文件 **180 用例**全绿；typecheck / lint **0 error**。
> P2 验证：mobile 22 文件 **184 用例** 全绿；P2 代码审查发现 1 阻断（BottomSheet worklet 在 sheetHeight 声明前捕获 → 键盘补偿失效）+ 3 应修，已全部修复后再提交。
> P3 验证：mobile 23 文件 **196 用例** / web 157 文件 **1036 用例** / `pnpm test:scripts` 46 个迁移全绿；审查无阻断、2 应修（picker 键盘下按钮不可达、自重重量被写成 0kg）+ 4 建议已修。
